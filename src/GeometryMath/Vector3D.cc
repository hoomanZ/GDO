...
{
  double norm = length();
  return Vector3D(d_vec[0]/norm, d_vec[1]/norm, d_vec[2]/norm);
}
...
/*Matrix3D
Vector3D::tensProduct(const Vector3D& vec)
{
  Matrix3D matrix(d_vec[0]*vec[0], d_vec[0]*vec[1], d_vec[0]*vec[2],
                  d_vec[1]*vec[0], d_vec[1]*vec[1], d_vec[1]*vec[2],
                  d_vec[2]*vec[0], d_vec[2]*vec[1], d_vec[2]*vec[2]);
  return matrix;
